...
import { PageKind, StepRoute } from '../../../utils/enums';
...
function ValidationResult({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (checkValidationResponse?.errorValidationCount > 0 && checkValidationResponse?.finished) {
  ...
  const showDetailTable = () => {
    showFileDetailsTableAction(true);
  };
