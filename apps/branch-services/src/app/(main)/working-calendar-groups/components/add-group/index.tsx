  ...
  return (
    <>
      <Form layout='vertical' form={form} onFinish={handleFinish}>
        <Box minHeight={'75vh'} flexDirection='column' justifyContent='space-between' padding={'3.2rem'}>
          ...
          <Box justifyContent='flex-end'>
            <Button htmlType='button' style={{ width: '10%' }} type='primaryOutlined' onClick={handleCancel}>
              {t('button.cancel')}
            </Button>
            <Button htmlType='submit' style={{ width: '10%' }} type='primary'>
              {t('create_group')}
            </Button>
          </Box>
        </Box>
      ...
      <ConfirmModal
        ...
        confirmLoading={createGroup.isPending || uploadGroupFile.isPending}
        groupName={form.getFieldValue('name')}
        unitCount={
          activeKeyTab === 'file-entry'
            ? uploadGroupFile.data?.unitCount ??
              uploadGroupFile.data?.totalRecords ??
              uploadGroupFile.data?.size ??
              uploadGroupFile.data?.units?.length ??
              0
            : form.getFieldValue('units')?.length ?? 0
        }
      />
    </>
  );
