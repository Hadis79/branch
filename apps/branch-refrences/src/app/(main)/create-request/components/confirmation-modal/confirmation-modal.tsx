...
const ConfirmationModal = (props) => {
  ...
  const { uploadResponse, checkValidationResponse } = useWidgetStore((state) => state);
  ...
  const isValidAmount = () => {
    ...
    return amountValue === checkValidationResponse?.fileUploadDto?.totalAmount;
  };
    ...
    <div className='modal-wrapper__content'>
      ...
      <div className={'modal-wrapper__transaction'}>
        ...
        <span className='transaction-amount'>
          {getValueOrDash(
            addThousandSeparator(checkValidationResponse?.fileUploadDto?.totalAmount as number) + ' ' + t('common.rial')
          )}{' '}
          -
          <span>
            {' '}
            {`${numberToPersian(rialToToman(checkValidationResponse?.fileUploadDto?.totalAmount as number))} ${t(
              'common.toman'
            )}`}
          </span>
        </span>
