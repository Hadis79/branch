...
// import useCheckValidationQuery from '../../queries/use-check-validation-query';
// import useClientSsn from './clientSsn';
...
const SubmitRequestStep = (clientSsn) => {
  ...
  const [desaibleCountinue, setDesaibleCountinue] = useState<boolean>(false);
  ...
  const { manualFetch } = useCheckValidationQuery(clientSsn, { manual: true });
  ...
  const handleBalance = (value) => {
    setBalance(value);
    value && setDesaibleCountinue(false);
  };
  ...
  const handleAccountError = (error) => {
    setMessage(error);
    error && setDesaibleCountinue(true);
  };
  ...
  return (
    <>
      ...
      {activeWithdrawalType === withdrawalTypesEnum.CHEQUE_METHOD && (
        <S.intermediaryAccountData>
          <div className='text-svg'>
            <InfoCircle />
            <div className='text'>{t('text_intermediary_accountData')}</div>
          </div>
        </S.intermediaryAccountData>
      )}
      <Form form={form} layout='vertical' onFinish={onFinish} name='submit-information'>
        ...
        <ActionButtonsContainer>
          ...
          <Button htmlType='submit' size='large' type='primary' disabled={desaibleCountinue}>
            {t('button.continue')}
          </Button>
        </ActionButtonsContainer>
      </Form>
