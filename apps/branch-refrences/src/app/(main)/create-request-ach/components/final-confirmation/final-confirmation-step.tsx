  ...
  function prepareResult() {
    ...
    !chequeMethodMood &&
      result.push({
        ...
        value: getValueOrDash(formValues?.accountBranchCode?.split('-')[0]),
      });
    chequeMethodMood &&
      result.push({
        type: 'item',
    ...
    result.push({
      ...
      value: ` ${formValues?.branchCode?.split('-')[0]} - ${formValues?.branchCode?.split('-')[1]} `,
    });
    result.push({
      type: 'item',
      title: t('field.account_branch_code'),
      value: getValueOrDash(formValues?.accountBranchCode?.split('-')[1]?.split(/(\s+)/)[6]),
    });
    result.push({
      type: 'item',
    ...
    result.push({
      ...
      value: (
        <div className='transaction_details_link' onClick={() => setOpenTransactionModal(true)}>
          <span className='upload_view_detail'>{t('view')} </span>
    ...
    result.push({
      type: 'header',
      title: t('field.user_information'),
      line: true,
    });

    result.push({
      type: 'item',
      title: t('field.shahab_code'),
      value: getValueOrDash(uploadResponse?.shahabCode),
    });
