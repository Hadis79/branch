...
const UploadFile = () => {
  ...
  const getPurposes = () => {
    ...
    const source = Array.isArray(statePurposes) ? statePurposes : Array.isArray(purposes) ? purposes : [];

    const data = source.filter((item) => item.transferType === purposesPaymentType);
    data?.map((item) => {
      return result.push({
        ...
        <Form form={form} layout='vertical' name='upload-file-form' onFinish={onFinish} autoComplete='off'>
          ...
          {!uploadResponse && (
            <S.UploadFileContainer fileRequiredError={fileRequiredError}>
              <Form.Item name='file' className='dragger-style' label={t('request_file')}>
                <Dragger
                  ...
                  // disabled={uploadFileLoading || !formValues?.accountBranchCode} TODO
          ...
          <Button style={{ marginBottom: '2.4rem', padding: 0 }} type='link' onClick={handleDownloadMenuClick}>
