...
import { useConfig, useResponsive, useUserQuery, useUserStore } from '@branch-services/hooks';
...
const UserSection = (props: DrawerProps) => {
  ...
  const { isLoading, refetch: refetchUser } = useUserQuery();
  ...
  const getUserFullName = () => {
    ...
    const persianFullName = `${user?.userInfo?.name} ${user?.userInfo?.family}`;
  ...
  const getOrgTitle = () => {
    ...
    const tooltip = (
      <>
        <div>{orgTitle}</div>
        <div>
          {t('field.org_ssn')} : {user?.branchInfo?.branch_name}
        </div>
      </>
    );
    return (
          ...
          <div className='user-info'>
            {isLoading ? (
              ...
              <>
                ...
                <span className='user-branch-info'>
                  {user?.branchInfo?.branch_name} - {user?.branchInfo?.branch_code}
                </span>
              </>
            )}
          </div>
